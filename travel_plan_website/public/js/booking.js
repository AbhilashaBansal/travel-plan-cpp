...
const plans = {};
plans["1"] = ["Europian Dream (London, Paris, Rome)", 128000];
plans["2"] = ["Europian Special (London, Paris, Switzerland)", 85000];
plans["3"] = ["Europian Tour (London, Berlin, Prague, Zurich)", 160000];
plans["4"] = ["Spanish Delight (Madrid, Barcelona, Valencia)", 60000];
plans["5"] = ["Japan & Korea (Tokyo, Seoul)", 145000];
plans["6"] = ["Asian Adventures (Tokyo, Manila, Singapore)", 125000];
plans["7"] = ["Australian Holiday (Sydney, Melbourne, Gold Coast, Adelaide)", 155000];
plans["8"] = ["South American Highlights (Rio, Argentina, Santiago, Iguazo Falls)", 95000];
plans["9"] = ["East Coast USA (New York, Washington DC, Niagra Falls)", 105000];
plans["10"] = ["USA Super Plan (New York, New Jersey, Washington DC, Niagra Falls, LA, Los Angeles)", 250000];
...
onload = function (){
    ...
    let id;
    let plan_pac_str = document.URL.split("?")[0].split("/");
    plan_pac_str = plan_pac_str[plan_pac_str.length-1];
    if(plan_pac_str=="bookplan"){

        //displaying shortest travel path
        ...
        document.getElementById("shortest-path").firstElementChild.textContent=str;
        people.addEventListener("input", function(){
            let cost = (distances[id]*2 + 20000)* Number(people.value);
            amt.value = cost;
        });
    }
    else{
        ...
        people.addEventListener("change", function(){
            let cost = plans[id][1] * Number(people.value);
            amt.value = cost;
        });
    }
